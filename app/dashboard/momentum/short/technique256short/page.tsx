...
export default function Technique256ShortPage() {
  const [selectedDate, setSelectedDate] = useState<Date | null>(null)
  const [data, setData] = useState<Technique256Short[]>([])
  ...
  useEffect(() => {
    async function fetchStats() {
      try {
        const response = await fetch('/api/momentum/technique256short?stats=true')
        if (response.ok) {
  ...
  useEffect(() => {
    async function fetchLatestDate() {
      try {
        const response = await fetch('/api/momentum/technique256short?latest=true')
        if (response.ok) {
          const result = await response.json()
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        const response = await fetch(`/api/momentum/technique256short?date=${dateStr}`)
        if (response.ok) {
          const result = await response.json()
          setData(result.data || [])
        }
      } catch (error) {
        console.error('Failed to fetch 256 Short data:', error)
      } finally {
        setLoading(false)
      }
    }
  ...
  const StockRow = ({ item }: { item: Technique256Short }) => (
    <tr 
  ...
  return (
    <div className="space-y-6">
      ...
      <AnalysisHeader
        title="256기법 분석 (단기)"
        description="3일선 60일선 돌파 및 역배열 조건에서의 단기 모멘텀 종목을 분석합니다"
      />
